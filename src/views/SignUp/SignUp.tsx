...
function SignUp() {
  ...
  const passwordConfirm = (e : string) => {
    const passwordRegex = /^(?=.*[a-zA-Z])(?=.*[!@#$%^*+=-])(?=.*[0-9]).{10,16}$/
    if (!passwordRegex.test(e)) {
      setPasswordMesage('숫자+영문자+특수문자 조합으로 10자리 이상 입력해주세요!');
              ...
              <div className="left">
                ...
                <div className="input_box  auth_input">
                  ...
                  <input
                    ...
                    onChange={(e) => nicknameConfirm(e.target.value)}
                  />
